Bitcoin Core versbitcoin 0.12.0 is now available from:

  <https://bitcoin.org/bin/bitcoin-core-0.12.0/>

This is a new major versbitcoin release, bringing new features and other improvements.
...
### Downgrade to a versbitcoin < 0.10.0
...
Signature validatbitcoin using libsecp256k1
---------------------------------------

ECDSA signatures inside Bitcoin transactbitcoins now use validatbitcoin using
[libsecp256k1](https://github.com/bitcoin-core/secp256k1) instead of OpenSSL.

Depending on the platform, this means a significant speedup for raw signature
validatbitcoin speed. The advantage is largest on x86_64, where validatbitcoin is over
five times faster. In practice, this translates to a raw reindexing and new
block validatbitcoin times that are less than half of what it was before.

Libsecp256k1 has undergone very extensive testing and validatbitcoin.
...
This optbitcoin can be specified in MiB per day and is turned off by default
(`-maxuploadtarget=0`).
...
A more detailed documentatbitcoin about keeping traffic low can be found in
...
reorganizatbitcoin, all new headers are sent, instead of just the new tip. This
can often prevent an extra roundtrip before the actual block is downloaded.

Memory pool limiting
--------------------

Previous versbitcoins of Bitcoin Core had their mempool limited by checking
a transactbitcoin's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactbitcoins paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versbitcoins of Bitcoin Core was to raise the default minimum
relay fee.

Bitcoin Core 0.12 will have a strict maximum size on the mempool. The
default value is 300 MB and can be configured with the `-maxmempool`
parameter. Whenever a transactbitcoin would cause the mempool to exceed
its maximum size, the transactbitcoin that (along with in-mempool descendants) has
...
size of unconfirmed transactbitcoin chains that are allowed in the mempool
(generally limiting the length of unconfirmed chains to 25 transactbitcoins, with a
total size of 101 KB).  These limits can be overriden using command line
arguments; see the extended help (`--help -help-debug`) for more informatbitcoin.

Opt-in Replace-by-fee transactbitcoins
----------------------------------

It is now possible to replace transactbitcoins in the transactbitcoin memory pool of
Bitcoin Core 0.12 nodes. Bitcoin Core will only allow replacement of
transactbitcoins which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transactbitcoin may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/bitcoin/bips/blob/master/bip-0125.mediawiki).

Transactbitcoin replacement can be disabled with a new command line optbitcoin,
`-mempoolreplacement=0`.  Transactbitcoins signaling replacement under BIP125 will
still be allowed into the mempool in this configuratbitcoin, but replacements will
be rejected.  This optbitcoin is intended for miners who want to continue the
transactbitcoin selectbitcoin behavior of previous releases.

The `-mempoolreplacement` optbitcoin is *not recommended* for wallet users seeking
to avoid receipt of unconfirmed opt-in transactbitcoins, because this optbitcoin does
not prevent transactbitcoins which are replaceable under BIP 125 from being accepted
...
a transactbitcoin is subject to replacement under BIP 125 should instead use the
updated RPC calls `gettransactbitcoin` and `listtransactbitcoins`, which now have an
additbitcoinal field in the output indicating if a transactbitcoin is replaceable under
BIP125 ("bip125-replaceable").

Note that the wallet in Bitcoin Core 0.12 does not yet have support for
creating transactbitcoins that would be replaceable under BIP 125.


RPC: Random-cookie RPC authenticatbitcoin
-------------------------------------

When no `-rpcpassword` is specified, the daemon now uses a special 'cookie'
file for authenticatbitcoin. This file is generated with random content when the
daemon starts, and deleted when it exits. Its contents are used as
authenticatbitcoin token. Read access to this file controls who can access through
RPC. By default it is stored in the data directory but its locatbitcoin can be
overridden with the optbitcoin `-rpccookiefile`.

This is similar to Tor's CookieAuthenticatbitcoin: see
https://www.torproject.org/docs/tor-manual.html.en

This allows running bitcoind without having to do any manual configuratbitcoin.

Relay: Any sequence of pushdatas in OP_RETURN outputs now allowed
-----------------------------------------------------------------

Previously OP_RETURN outputs with a payload were only relayed and mined if they
had a single pushdata. This restrictbitcoin has been lifted to allow any
combinatbitcoin of data pushes and numeric constant opcodes (OP_1 to OP_16) after
...
Relay and Mining: Priority transactbitcoins
---------------------------------------

Bitcoin Core has a heuristic 'priority' based on coin value and age. This
calculatbitcoin is used for relaying of transactbitcoins which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactbitcoins for mined blocks. Bitcoin Core will relay transactbitcoins with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Bitcoin Core 0.12, when mempool limit has been reached a higher minimum
relay fee takes effect to limit memory usage. Transactbitcoins which do not meet
this higher effective minimum relay fee will not be relayed or mined even if
they rank highly according to the priority heuristic.

The mining of transactbitcoins based on their priority is also now disabled by
default. To re-enable it, simply set `-blockprioritysize=<n>` where is the size
in bytes of your blocks to reserve for these transactbitcoins. The old default was
50k, so to retain approximately the same policy, you would set
`-blockprioritysize=50000`.

Additbitcoinally, as a result of computatbitcoinal simplificatbitcoins, the priority value
used for transactbitcoins received with unconfirmed inputs is lower than in prior
versbitcoins due to avoiding recomputing the amounts as input transactbitcoins confirm.

External miner policy set via the `prioritisetransactbitcoin` RPC to rank
transactbitcoins already in the mempool continues to work as it has previously.
Note, however, that if mining priority transactbitcoins is left disabled, the
priority delta will be ignored and only the fee metric will be effective.

This internal automatic prioritizatbitcoin handling is being considered for removal
entirely in Bitcoin Core 0.13, and it is at this time undecided whether the
more accurate priority calculatbitcoin for chained unconfirmed transactbitcoins will be
restored. Community directbitcoin on this topic is particularly requested to help
set project priorities.

Automatically use Tor hidden services
-------------------------------------

Starting with Tor versbitcoin 0.2.7.1 it is possible, through Tor's control socket
API, to create and destroy 'ephemeral' hidden services programmatically.
Bitcoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorizatbitcoin is available),
Bitcoin Core automatically creates a hidden service to listen on, without
manual configuratbitcoin. Bitcoin Core will also use Tor automatically to connect
to other .onbitcoin nodes if the control socket can be successfully opened. This
will positively affect the number of available .onbitcoin nodes and their usage.

This new feature is enabled by default if Bitcoin Core is listening, and
a connectbitcoin to Tor can be made. It can be configured with the `-listenonbitcoin`,
`-torcontrol` and `-torpassword` settings. To show verbose debugging
informatbitcoin, pass `-debug=tor`.

Notificatbitcoins through ZMQ
-------------------------

Bitcoind can now (optbitcoinally) asynchronously notify clients through a
ZMQ-based PUB socket of the arrival of new transactbitcoins and blocks.
This feature requires installatbitcoin of the ZMQ C API library 4.x and
configuring its use through the command line or configuratbitcoin file.
Please see [docs/zmq.md](/doc/zmq.md) for details of operatbitcoin.

Wallet: Transactbitcoin fees
------------------------

Various improvements have been made to how the wallet calculates
transactbitcoin fees.
...
Based on past transactbitcoin data, floating fees approximate the fees
required to get into the `m`th block from now. This is configurable
with `-txconfirmtarget=<m>` (default: `2`).

Sometimes, it is not possible to give good estimates, or an estimate
at all. Therefore, a fallback value can be set with `-fallbackfee=<f>`
(default: `0.0002` ION/kB).

At all times, Bitcoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) ION.
Furthermore, Bitcoin Core will never create transactbitcoins paying less than
the current minimum relay fee.
Finally, a user can set the minimum fee rate for all transactbitcoins with
`-mintxfee=<i>`, which defaults to 1000 satoshis per kB.

Wallet: Negative confirmatbitcoins and conflict detectbitcoin
-----------------------------------------------------

The wallet will now report a negative number for confirmatbitcoins that indicates
how deep in the block chain the conflict is found. For example, if a transactbitcoin
A has 5 confirmatbitcoins and spends the same input as a wallet transactbitcoin B, B
will be reported as having -5 confirmatbitcoins. If another wallet transactbitcoin C
spends an output from B, it will also be reported as having -5 confirmatbitcoins.
To detect conflicts with historical transactbitcoins in the chain a one-time
`-rescan` may be needed.

Unlike earlier versbitcoins, unconfirmed but non-conflicting transactbitcoins will never
get a negative confirmatbitcoin count. They are not treated as spendable unless
they're coming from ourself (change) and accepted into our local mempool,
however. The new "trusted" field in the `listtransactbitcoins` RPC output
indicates whether outputs of an unconfirmed transactbitcoin are considered
spendable.

Wallet: Merkle branches removed
-------------------------------

Previously, every wallet transactbitcoin stored a Merkle branch to prove its
presence in blocks. This wasn't being used for more than an expensive
sanity check. Since 0.12, these are no longer stored. When loading a
0.12 wallet into an older versbitcoin, it will automatically rescan to avoid
failed checks.

Wallet: Pruning
---------------

With 0.12 it is possible to use wallet functbitcoinality in pruned mode.
...
reorganizatbitcoins of more than 2 days - which are unlikely to happen in
practice. In future releases, a higher value may also help the network
as a whole: stored blocks could be served to other nodes.

For further informatbitcoin about pruning, you may also consult the [release
...
versbitcoin to allow peers to identify old nodes which allow bloom filtering of the
connectbitcoin despite lacking the new service bit.

In this versbitcoin, it is only enforced for peers that send protocol versbitcoins
`>=70011`. For the next major versbitcoin it is planned that this restrictbitcoin will be
removed. It is recommended to update SPV clients to check for the `NODE_BLOOM`
service bit for nodes that report versbitcoins newer than 70011.

Optbitcoin parsing behavior
-----------------------

Command line optbitcoins are now parsed strictly in the order in which they are
specified. It used to be the case that `-X -noX` ends up, unintuitively, with X
set, as `-X` had precedence over `-noX`. This is no longer the case. Like for
other software, the last specified value for an optbitcoin will hold.
...
The following items contain assembly representatbitcoins of scriptSig signatures
and are affected by this change:

- RPC `getrawtransactbitcoin`
- RPC `decoderawtransactbitcoin`
- RPC `decodescript`
- REST `/rest/tx/` (JSON format)
- REST `/rest/block/` (JSON format when including extended tx details)
- `bitcoin-tx -json`

For example, the `scriptSig.asm` property of a transactbitcoin input that
previously showed an assembly representatbitcoin of:
...
SSL support for RPC, previously enabled by the optbitcoin `rpcssl` has been dropped
from both the client and the server. This was done in preparatbitcoin for removing
...
migratbitcoin path is to use `stunnel`. This is an utility that can tunnel
arbitrary TCP connectbitcoins inside SSL. On e.g. Ubuntu it can be installed with:

    sudo apt-get install stunnel4

Then, to tunnel a SSL connectbitcoin on 28332 to a RPC server bound on localhost on port 18332 do:

    stunnel -d 28332 -r 127.0.0.1:18332 -p stunnel.pem -P ''

It can also be set up system-wide in inetd style.

Another way to re-attain SSL would be to setup a httpd reverse proxy. This solutbitcoin
would allow the use of different authenticatbitcoin, loadbalancing, on-the-fly compressbitcoin and
caching. A sample config for apache2 could look like:
    ...
    <Locatbitcoin /bitcoinrpc>
        ProxyPass http://127.0.0.1:8332/
        ProxyPassReverse http://127.0.0.1:8332/
        # optbitcoinal enable digest auth
        # AuthType Digest
        # ...

        # optbitcoinal bypass bitcoind rpc basic auth
        # RequestHeader set Authorizatbitcoin "Basic <hash>"
...
memory. As part of these changes, consensus critical calculatbitcoins are cached on a
transactbitcoin's acceptance into the mempool and the mining code now relies on the
...
- #6234 `d38cd47` fix rpcmining/getblocktemplate univalue transitbitcoin logic error (Jonas Schnelli)
...
- #6262 `247b914` Return all available informatbitcoin via RPC call "validateaddress" (dexX7)
...
- #6379 `c52e8b3` rpc: Accept scientific notatbitcoin for monetary amounts in JSON (Wladimir J. van der Laan)
- #6388 `fd5dfda` rpc: Implement random-cookie based authenticatbitcoin (Wladimir J. van der Laan)
...
- #6346 `6bb2805` Add OP_RETURN support in createrawtransactbitcoin RPC call, add tests. (paveljanik)
...
- #5264 `48efbdb` show scriptSig signature hash types in transactbitcoin decodes. fixes #3166 (mruddy)
...
- #5936 `bf7c195` [RPC] Add optbitcoinal locktime to createrawtransactbitcoin (Tom Harding)
...
- #7087 `be281d8` [Net]Add -enforcenodebloom optbitcoin (Patrick Strateman)
- #7044 `438ee59` RPC: Added additbitcoinal config optbitcoin for multiple RPC users. (Gregory Sanders)
- #7072 `c143c49` [RPC] Add transactbitcoin size to JSON output (Nikita Zhavoronkov)
...
- #7312 `fd4bd50` Add RPC call abandontransactbitcoin (Alex Morcos)
- #7222 `e25b158` RPC: indicate which transactbitcoins are replaceable (Suhas Daftuar)
...
### Configuratbitcoin and command-line optbitcoins

- #6164 `8d05ec7` Allow user to use -debug=1 to enable all debugging (lpescher)
- #5288 `4452205` Added `-whiteconnectbitcoins=<n>` optbitcoin (Josh Lehan)
...
- #6881 `2b62551` Debug: Add optbitcoin for microsecond precisbitcoin in debug.log (Suhas Daftuar)
- #6776 `e06c14f` Support -checkmempool=N, which runs checks once every N transactbitcoins (Pieter Wuille)
- #6896 `d482c0a` Make -checkmempool=1 not fail through int32 overflow (Pieter Wuille)
- #6993 `b632145` Add -blocksonly optbitcoin (Patrick Strateman)
- #7323 `a344880` 0.12: Backport -bytespersigop optbitcoin (Luke-Jr)
- #7386 `da83ecd` Add optbitcoin `-permitrbf` to set transactbitcoin replacement policy (Wladimir J. van der Laan)
- #7290 `b16b5bc` Add missing optbitcoins help (MarcoFalke)
...
- #6222 `9c93ee5` Explicitly set tx.nVersbitcoin for the genesis block and mining tests (Mark Friedenbach)
- #5985 `3a1d3e8` Fix removing of orphan transactbitcoins (Alex Morcos)
- #6221 `dd8fe82` Prune: Support noncontiguous block files (Adam Weiss)
- #6124 `41076aa` Mempool only CHECKLOCKTIMEVERIFY (BIP65) verificatbitcoin, unparameterized versbitcoin (Peter Todd)
- #6329 `d0a10c1` acceptnonstdtxn optbitcoin to skip (most) "non-standard transactbitcoin" checks, for testnet/regtest only (Luke-Jr)
- #6410 `7cdefb9` Implement accurate memory accounting for mempool (Pieter Wuille)
- #6444 `24ce77d` Exempt unspendable transactbitcoin outputs from dust checks (dexX7)
- #5913 `a0625b8` Add absurdly high fee message to validatbitcoin state (Shaul Kfir)
...
- #6464 `8f3b3cd` Always clean up manual transactbitcoin prioritizatbitcoin (Casey Rodarmor)
- #6865 `d0badb9` Fix chainstate serialized_size computatbitcoin (Pieter Wuille)
- #6566 `ff057f4` BIP-113: Mempool-only median time-past as endpoint for lock-time calculatbitcoins (Mark Friedenbach)
...
- #5967 `05d5918` Alter assumptbitcoins in CCoinsViewCache::BatchWrite (Alex Morcos)
- #6871 `0e93586` nSequence-based Full-RBF opt-in (Peter Todd)
- #7008 `eb77416` Lower bound priority (Alex Morcos)
- #6915 `2ef5ffa` [Mempool] Improve removal of invalid transactbitcoins after reorgs (Suhas Daftuar)
...
- #7062 `12c469b` [Mempool] Fix mempool limiting and replace-by-fee for PrioritiseTransactbitcoin (Suhas Daftuar)
...
- #6256 `8ccc07c` Use best header chain timestamps to detect partitbitcoining (Gavin Andresen)
- #6283 `a903ad7` make CAddrMan::size() return the correct type of size_t (Diapolo)
- #6272 `40400d5` Improve proxy initializatbitcoin (continues #4871) (Wladimir J. van der Laan, Diapolo)
...
- #6498 `219b916` Keep track of recently rejected transactbitcoins with a rolling bloom filter (cont'd) (Peter Todd)
...
- #6583 `af9305a` add support for miniupnpc api versbitcoin 14 (Pavel Vasin)
- #6374 `69dc5b5` Connectbitcoin slot exhaustbitcoin DoS mitigatbitcoin (Patrick Strateman)
- #6636 `536207f` net: correctly initialize nMinPingUsecTime (Wladimir J. van der Laan)
- #6579 `0c27795` Add NODE_BLOOM service bit and bump protocol versbitcoin (Matt Corallo)
...
- #7179 `44fef99` net: Fix sent reject messages for blocks and transactbitcoins (Wladimir J. van der Laan)
...
- #6519 `87f37e2` Make logging for validatbitcoin optbitcoinal (Wladimir J. van der Laan)
- #6351 `2a1090d` CHECKLOCKTIMEVERIFY (BIP65) IsSuperMajority() soft-fork (Peter Todd)
- #6931 `54e8bfe` Skip BIP 30 verificatbitcoin where not necessary (Alex Morcos)
- #6954 `e54ebbf` Switch to libsecp256k1-based ECDSA validatbitcoin (Pieter Wuille)
...
- #7500 `889e5b3` Correctly report high-S violatbitcoins (Pieter Wuille)


### Build system

- #6210 `0e4f2a0` build: disable optbitcoinal use of gmp in internal secp256k1 build (Wladimir J. van der Laan)
...
- #7092 `348b281` build: Set osx permissbitcoins in the dmg to make Gatekeeper happy (Cory Fields)
...
- #6088 `91389e5` fundrawtransactbitcoin (Matt Corallo)
- #6415 `ddd8d80` Implement watchonly support in fundrawtransactbitcoin (Matt Corallo)
- #6567 `0f0f323` Fix crash when mining with empty keypool. (Daniel Kraft)
- #6688 `4939eab` Fix locking in GetTransactbitcoin. (Alex Morcos)
...
- #5924 `12a7712` Clean up change computatbitcoin in CreateTransactbitcoin. (Daniel Kraft)
- #6906 `48b5b84` Reject invalid pubkeys when reading ckey items from the wallet. (Gregory Maxwell)
- #7010 `e0a5ef8` Fix fundrawtransactbitcoin handling of includeWatching (Peter Todd)
- #6851 `616d61b` Optimisatbitcoin: Store transactbitcoin list order in memory rather than compute it every need (Luke-Jr)
- #6134 `e92377f` Improve usage of fee estimatbitcoin code (Alex Morcos)
...
- #7096 `9490bd7` [Wallet] Improve minimum absolute fee GUI optbitcoins (Jonas Schnelli)
- #6216 `83f06ca` Take the training wheels off anti-fee-sniping (Peter Todd)
- #4906 `96e8d12` Issue#1643: Coinselectbitcoin prunes extraneous inputs from ApproximateBestSubset (Murch)
- #7200 `06c6a58` Checks for null data transactbitcoin before issuing error to debug.log (Andy Craze)
- #7296 `a36d79b` Add sane fallback for fee estimatbitcoin (Alex Morcos)
- #7293 `ff9b610` Add regressbitcoin test for vValue sort order (MarcoFalke)
...
- #4587 `0c465f5` allow users to set -onbitcoin via GUI (Diapolo)
- #6529 `c0f66ce` show client user agent in debug window (Diapolo)
- #6594 `878ea69` Disallow duplicate windows. (Casey Rodarmor)
- #5665 `6f55cdd` add verifySize() functbitcoin to PaymentServer (Diapolo)
- #6317 `ca5e2a1` minor optimisatbitcoins in peertablemodel (Diapolo)
...
- #7051 `a190777` ui: Add "Copy raw transactbitcoin data" to transactbitcoin list context menu (Wladimir J. van der Laan)
- #6979 `776848a` simple mempool info in debug window (Jonas Schnelli)
- #7006 `26af1ac` add startup optbitcoin to reset Qt settings (Jonas Schnelli)
- #6780 `2a94cd6` Call init's parameter interactbitcoin before we create the UI optbitcoins model (Jonas Schnelli)
...
- #7327 `b16b5bc` [Wallet] Transactbitcoin View: LastMonth calculatbitcoin fixed (crowning-)
...
- #6417 `44fa82d` [QA] fix possible reorg issue in (fund)rawtransactbitcoin(s).py RPC test (Jonas Schnelli)
...
- #6534 `6c1c7fd` Fix test locking issues and un-revert the probable-deadlines assertbitcoins commit (Cory Fields)
- #6509 `bb4faee` Fix race conditbitcoin on test node shutdown (Casey Rodarmor)
...
- #6616 `5ab5dca` Regressbitcoin Tests: Migrated rpc-tests.sh to all Python rpc-tests.py (Peter Tschipper)
...
- #6848 `f2c869a` Add DERSIG transactbitcoin test cases (Ross Nicoll)
- #6813 `5242bb3` Support gathering code coverage data for RPC tests with lcov (dexX7)
- #6888 `c8322ff` Clear strMiscWarning before running PartitbitcoinAlert (Eric Lombrozo)
...
- #7095 `d8368a0` Replace scriptnum_test's normative ScriptNum implementatbitcoin (Wladimir J. van der Laan)
- #7063 `6abf6eb` [Tests] Add prioritisetransactbitcoin RPC test (Suhas Daftuar)
...
- #7170 `453c567` tests: Disable Tor interactbitcoin (Wladimir J. van der Laan)
...
- #6213 `e54ff2f` [init] add -blockversbitcoin help and extend -upnp help (Diapolo)
...
- #6430 `b01981e` doc: add documentatbitcoin for shared library libbitcoinconsensus (Braydon Fuller)
- #6372 `dcc495e` Update Linearize tool to support Windows paths; fix variable scope; update README and example configuratbitcoin (Paul Georgiou)
- #6453 `8fe5cce` Separate core memory usage computatbitcoin in core_memusage.h (Pieter Wuille)
...
- #6504 `b6fee6b` Ratbitcoinalize currency unit to "ION" (Ross Nicoll)
...
- #6103 `13b8282` Add ZeroMQ notificatbitcoins (João Barbosa)
...
- #7114 `f3d0fdd` util: Don't set strMiscWarning on every exceptbitcoin (Wladimir J. van der Laan)
