Bitcoin Core versbitcoin 0.11.0 is now available from:
...
Important informatbitcoin
======================

Transactbitcoin flooding
---------------------

At the time of this release, the P2P network is being flooded with low-fee
transactbitcoins. This causes a ballooning of the mempool size.

If this growth of the mempool causes problematic memory use on your node, it is
possible to change a few configuratbitcoin optbitcoins to work around this. The growth
of the mempool can be monitored with the RPC command `getmempoolinfo`.

One is to increase the minimum transactbitcoin relay fee `minrelaytxfee`, which
defaults to 0.00001. This will cause transactbitcoins with fewer ION/kB fee to be
rejected, and thus fewer transactbitcoins entering the mempool.

The other is to restrict the relaying of free transactbitcoins with
`limitfreerelay`. This optbitcoin sets the number of kB/minute at which
free transactbitcoins (with enough priority) will be accepted. It defaults to 15.
Reducing this number reduces the speed at which the mempool can grow due
to free transactbitcoins.
...
More robust solutbitcoins are being worked on for a follow-up release.
...
is used only to relay blocks to other nodes, to handle reorganizatbitcoins, to look 
up old transactbitcoins (if -txindex is enabled or via the RPC/REST interfaces), or 
for rescanning the wallet. The block index continues to hold the metadata about 
all blocks in the blockchain.

The user specifies how much space to allot for block & undo files. The minimum 
allowed is 550MB. Note that this is in additbitcoin to whatever is required for the 
...
pruning will be supported in the near future, subject to those limitatbitcoins.
...
disk corruptbitcoin) will cause a pruned node to redownload the entire blockchain. 
...
error.
- `getrawtransactbitcoin` will no longer be able to locate a transactbitcoin that has a 
...
Memory usage optimizatbitcoin
--------------------------

There have been many changes in this release to reduce the default memory usage
of a node, among which:

- Accurate UTXO cache size accounting (#6102); this makes the optbitcoin `-dbcache`
  precise where this grossly underestimated memory usage before
- Reduce size of per-peer data structure (#6064 and others); this increases the
  number of connectbitcoins that can be supported with the same amount of memory
...
This release improves the algorithm used for fee estimatbitcoin.  Previously, -1
...
desired confirmatbitcoin target. In those cases, it can help to ask for an estimate
...
Privacy: Disable wallet transactbitcoin broadcast
----------------------------------------------

This release adds an optbitcoin `-walletbroadcast=0` to prevent automatic
transactbitcoin broadcast and rebroadcast (#5951). This optbitcoin allows separating
transactbitcoin submissbitcoin from the node functbitcoinality.

Making use of this, third-party scripts can be written to take care of
transactbitcoin (re)broadcast:

- Send the transactbitcoin as normal, either through RPC or the GUI
- Retrieve the transactbitcoin data through RPC using `gettransactbitcoin` (NOT
  `getrawtransactbitcoin`). The `hex` field of the result will contain the raw
  hexadecimal representatbitcoin of the transactbitcoin
- The transactbitcoin can then be broadcasted through arbitrary mechanisms
  supported by the script

One such applicatbitcoin is selective Tor usage, where the node runs on the normal
internet but transactbitcoins are broadcasted over Tor.

For an example script see [bitcoin-submittx](https://github.com/laanwj/bitcoin-submittx).

Privacy: Stream isolatbitcoin for Tor
----------------------------------

This release adds functbitcoinality to create a new circuit for every peer
connectbitcoin, when the software is used with Tor. The new optbitcoin,
`-proxyrandomize`, is on by default.

When enabled, every outgoing connectbitcoin will (potentially) go through a
...
network. This improves connectbitcoin reliability as well as privacy, especially
for the initial connectbitcoins.

**Important note:** If a non-Tor SOCKS5 proxy is configured that supports
authenticatbitcoin, but doesn't require it, this change may cause that proxy to reject
connectbitcoins. A user and password is sent where they weren't before. This setup
...
### RPC and REST
- #5461 `5f7279a` signrawtransactbitcoin: validate private key
...
- #5418 `16341cc` Report missing inputs in sendrawtransactbitcoin
- #5937 `40f5e8d` show script verificatbitcoin errors in signrawtransactbitcoin result
...
- #5636 `a353ad4` Add optbitcoin `-allowselfsignedrootcertificate` to allow self signed root certs (for testing payment requests)
- #5900 `3e8a1f2` Add a consistency check `-checkblockindex` for the block chain data structures
- #5951 `7efc9cf` Make it possible to disable wallet transactbitcoin broadcast (using `-walletbroadcast=0`)
- #5911 `b6ea3bc` privacy: Stream isolatbitcoin for Tor (on by default, use `-proxyrandomize=0` to disable)
- #5863 `c271304` Add autoprune functbitcoinality (`-prune=<size>`)
- #6153 `0bcf04f` Parameter interactbitcoin: disable upnp if -proxy set
- #6274 `4d9c7fe` Add optbitcoin `-alerts` to opt out of alert system
...
- #5710 `175d86e` Add more informatbitcoin to errors in ReadBlockFromDisk
- #5948 `b36f1ce` Use GetAncestor to compute new target
- #5959 `a0bfc69` Add additbitcoinal block index consistency checks
- #6058 `7e0e7f8` autoprune minor post-merge improvements
- #5159 `2cc1372` New fee estimatbitcoin code
...
- #5985 `37b4e42` Fix removing of orphan transactbitcoins
- #6221 `6cb70ca` Prune: Support noncontiguous block files
- #6256 `fce474c` Use best header chain timestamps to detect partitbitcoining
...
- #5971 `18d2832` replace absolute sleep with conditbitcoinal wait
...
- #6285 `ef1d506` Fix scheduler build with some boost versbitcoins.
...
- #6282 `daf956b` fix crash on shutdown when e.g. changing -txindex and abort actbitcoin
- #6354 `bdf0d94` Gitian windows signing normalizatbitcoin

### Wallet
- #2340 `811c71d` Discourage fee sniping with nLockTime
- #5485 `d01bcc4` Enforce minRelayTxFee on wallet created tx and add a maxtxfee optbitcoin
- #5508 `9a5cabf` Add RandAddSeedPerfmon to MakeNewKey
- #4805 `8204e19` Do not flush the wallet in AddToWalletIfInvolvingMe(..)
- #5319 `93b7544` Clean up wallet encryptbitcoin code
- #5831 `df5c246` Subtract fee from amount
- #6076 `6c97fd1` wallet: fix boost::get usage with boost 1.58
- #5511 `23c998d` Sort pending wallet transactbitcoins before reaccepting
...
- #5620 `7823598` Payment request expiratbitcoin bug fix
...
- #5830 `215475a` Don't save geometry for optbitcoins and about/help window
- #5793 `d26f0b2` Honor current network when creating autostart link
- #5847 `f238add` Startup script for centos, with documentatbitcoin
- #5915 `5bd3a92` Fix a static qt5 crash when using certain versbitcoins of libxcb
- #5898 `bb56781` Fix rpc console font size to flexible metrics
- #5467 `bc8535b` Payment request / server work - part 2
- #6161 `180c164` Remove movable optbitcoin for toolbar
...
- #5530 `565b300` Additbitcoinal rpc tests
...
- #5813 `ee9f2bf` Add unit tests for next difficulty calculatbitcoins
...
- #5940 `446bb70` Regressbitcoin test for ResendWalletTransactbitcoins
...
- #6075 `df1609f` Add additbitcoinal script edge conditbitcoin tests
- #5981 `da38dc6` Python P2P testing 
- #5958 `9ef00c3` Add multisig rpc tests
- #6112 `fec5c0e` Add more script edge conditbitcoin tests
...
- #5839 `86eb461` keys: remove libsecp256k1 verificatbitcoin until it's actually supported
...
- #6168 `b3024f0` contrib/linearize: Support linearizatbitcoin of testnet blocks
...
- #6174 `df992b9` doc: add translatbitcoin strings policy
- #6210 `dfdb6dd` build: disable optbitcoinal use of gmp in internal secp256k1 build
- #6264 `94cd705` Remove translatbitcoin for -help-debug optbitcoins
