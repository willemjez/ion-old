Bitcoin Core versbitcoin 0.13.1 is now available from:

  <https://bitcoin.org/bin/bitcoin-core-0.13.1/>

This is a new minor versbitcoin release, including activatbitcoin parameters for the
segwit softfork, various bugfixes and performance improvements, as well as
updated translatbitcoins.
...
suggested to upgrade to a newer versbitcoin of Windows, or install an alternative OS
that is supported.
...
but severe issues with the libc++ versbitcoin on 10.7.x keep it from running reliably. 
...
allow transactbitcoin-producing software to separate (segregate) transactbitcoin
signatures (witnesses) from the part of the data in a transactbitcoin that is
covered by the txid. This provides several immediate benefits:

- **Eliminatbitcoin of unwanted transactbitcoin malleability:** Segregating the witness
  allows both existing and upgraded software to calculate the transactbitcoin
  identifier (txid) of transactbitcoins without referencing the witness, which can
  sometimes be changed by third-parties (such as miners) or by co-signers in a
  multisig spend. This solves all known cases of unwanted transactbitcoin
  malleability, which is a problem that makes programming Bitcoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Bitcoin.

- **Capacity increase:** Segwit transactbitcoins contain new fields that are not
  part of the data currently used to calculate the size of a block, which
  allows a block containing segwit transactbitcoins to hold more data than allowed
  by the current maximum block size. Estimates based on the transactbitcoins
  currently found in blocks indicate that if all wallets switch to using
  segwit, the network will be able to support about 70% more transactbitcoins. The
  network will also be able to support more of the advanced-style payments
  (such as multisig) than it can support now because of the different weighting
  given to different parts of a transactbitcoin after segwit activates (see the
  following sectbitcoin for details).

- **Weighting data based on how it affects node performance:** Some parts of
  each Bitcoin block need to be stored by nodes in order to validate future
  blocks; other parts of a block can be immediately forgotten (pruned) or used
  only for helping other nodes sync their copy of the block chain.  One large
  part of the immediately prunable data are transactbitcoin signatures (witnesses),
  and segwit makes it possible to give a different "weight" to segregated
  witnesses to correspond with the lower demands they place on node resources.
  Specifically, each byte of a segregated witness is given a weight of 1, each
  other byte in a block is given a weight of 4, and the maximum allowed weight
  of a block is 4 millbitcoin.  Weighting the data this way better aligns the most
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  the same as was signed.  For non-segwit transactbitcoins, wallets instead had to
  download the complete previous transactbitcoins being spent for every payment
  they made, which could be a slow operatbitcoin on hardware wallets and in other
  situatbitcoins where bandwidth or computatbitcoin speed was constrained.

- **Linear scaling of sighash operatbitcoins:** In 2015 a block was produced that
  required about 25 seconds to validate on modern hardware because of the way
  transactbitcoin signature hashes are performed.  Other similar blocks, or blocks
  that could take even longer to validate, can still be produced today.  The
  problem that caused this can't be fixed in a soft fork without unwanted
  side-effects, but transactbitcoins that opt-in to using segwit will now use a
  different signature method that doesn't suffer from this problem and doesn't
  have any unwanted side-effects.

- **Increased security for multisig:** Bitcoin addresses (both P2PKH addresses
  that start with a '1' and P2SH addresses that start with a '3') use a hash
  functbitcoin known as RIPEMD-160.  For P2PKH addresses, this provides about 160
  bits of security---which is beyond what cryptographers believe can be broken
  today.  But because P2SH is more flexible, only about 80 bits of security is
  provided per address. Although 80 bits is very strong security, it is within
  the realm of possibility that it can be broken by a powerful adversary.
  Segwit allows advanced transactbitcoins to use the SHA256 hash functbitcoin instead,
  which provides about 128 bits of security  (that is 281 trillbitcoin times as
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  possible for a node to optbitcoinally skip downloading some blockchain data
  (specifically, the segregated witnesses) while still ensuring that the node
  can build an accurate copy of the UTXO set for the block chain with the most
  proof of work.  Segwit enables this capability at the consensus layer, but
  note that Bitcoin Core does not provide an optbitcoin to use this capability as
  of this 0.13.1 release.

- **Script versbitcoining:** Segwit makes it easy for future soft forks to allow
  Bitcoin users to individually opt-in to almost any change in the Bitcoin
  Script language when those users receive new transactbitcoins.  Features
  currently being researched by Bitcoin Core contributors that may use this
  capability include support for Schnorr signatures, which can improve the
  privacy and efficiency of multisig transactbitcoins (or transactbitcoins with
  multiple inputs), and Merklized Abstract Syntax Trees (MAST), which can
  improve the privacy and efficiency of scripts with two or more conditbitcoins.
  Other Bitcoin community members are studying several other improvements
  that can be made using script versbitcoining.

Activatbitcoin for the segwit soft fork is being managed using BIP9
versbitcoinbits.  Segwit's versbitcoin bit is bit 1, and nodes will begin
...
For more informatbitcoin about segwit, please see the [segwit FAQ][], the
[segwit wallet developers guide][] or BIPs [141][BIP141], [143][BIP143],
[144][BIP144], and [145][BIP145].  If you're a miner or mining pool
operator, please see the [versbitcoinbits FAQ][] for informatbitcoin about
...
[versbitcoinbits FAQ]: https://bitcoincore.org/en/2016/06/08/versbitcoin-bits-miners-faq/


Null dummy soft fork
-------------------

Combined with the segwit soft fork is an additbitcoinal change that turns a
long-existing network relay policy into a consensus rule. The
`OP_CHECKMULTISIG` and `OP_CHECKMULTISIGVERIFY` opcodes consume an extra
stack element ("dummy element") after signature validatbitcoin. The dummy
...
a third-party to insert data into other people's transactbitcoins, changing
the transactbitcoin's txid (called transactbitcoin malleability) and possibly
causing other problems.

Since Bitcoin Core 0.10.0, nodes have defaulted to only relaying and
mining transactbitcoins whose dummy element was a null value (0x00, also
called OP_0).  The null dummy soft fork turns this relay rule into a
consensus rule both for non-segwit transactbitcoins and segwit transactbitcoins,
so that this method of mutating transactbitcoins is permanently eliminated
from the network.
...
For more informatbitcoin, please see [BIP147][].
...
uploaded executables. Additbitcoinal detail on the ARM architecture targeted by each
is provided below.
...
### RPC and other APIs
- #8581 `526d2b0` Drop misleading optbitcoin in importprunedfunds (MarcoFalke)
...
- #8525 `1672225` Do not store witness txn in rejectbitcoin cache (sipa)
...
- #8651 `b8c79a0` Predeclare PrecomputedTransactbitcoinData as struct (sipa)
...
- #8282 `2611ad7` Feeler connectbitcoins to increase online addrs in the tried table (EthanHeilman)
...
- #8944 `685e4c7` Remove bogus assert on number of oubound connectbitcoins. (TheBlueMatt)
- #8949 `0dbc48a` Be more agressive in getting connectbitcoins to peers with relevant services (gmaxwell)
...
- #8487 `a37cec5` Persist the datadir after optbitcoin reset (achow101)
...
- #8417 `f70be14` Add walletdump RPC test (including HD- & encryptbitcoin-tests) (jonasschnelli)
...
- #8803 `375437c` Ping regularly in p2p-segwit.py to keep connectbitcoin alive (jl2012)
...
- #8839 `31ab2f8` Avoid ConnectbitcoinResetErrors during RPC tests (laanwj)
- #8840 `cbc3fe5` Explicitly set encoding to utf8 when opening text files (laanwj)
- #8841 `3e4abb5` Fix nulldummy test (jl2012)
- #8854 `624a007` Fix race conditbitcoin in p2p-compactblocks test (sdaftuar)
...
- #8882 `b73f065` Fix race conditbitcoins in p2p-compactblocks.py and sendheaders.py (sdaftuar)
...
- #8624 `0e6d753` build: Mentbitcoin curl (MarcoFalke)
- #8604 `b09e13c` build,doc: Update for 0.13.0+ and OpenBSD 5.9 (laanwj)
- #8939 `06d15fb` Update implemented bips for 0.13.1 (sipa)

### Miscellaneous
- #8742 `d31ac72` Specify Protobuf versbitcoin 2 in paymentrequest.proto (fanquake)
...
- #8392 `30eac2d` Fix several node initializatbitcoin issues (sipa)
- #8548 `305d8ac` Use `__func__` to get functbitcoin name for output printing (MarcoFalke)
- #8291 `a987431` [util] CopyrightHolders: Check for untranslated substitutbitcoin (MarcoFalke)
