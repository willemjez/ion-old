Bitcoin Core versbitcoin 0.9.3 is now available from:

  https://bitcoin.org/bin/0.9.3/

This is a new minor versbitcoin release, bringing only bug fixes and updated
translatbitcoins. Upgrading to this release is recommended.
...
- Avoid querying DNS seeds, if we have open connectbitcoins
- Remove a useless millisleep in socket handler
- Stricter memory limits on CNode
- Better orphan transactbitcoin handling
- Add `-maxorphantx=<n>` and `-maxorphanblocks=<n>` optbitcoins for control over the maximum orphan transactbitcoins and blocks
...
- Fix boost detectbitcoin in build system on some platforms
