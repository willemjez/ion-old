Bitcoin Core versbitcoin 0.13.0 is now available from:
...
In additbitcoin to that, with 0.12.x there have been varied reports of Bitcoin Core
...
suggested to upgrade to a newer verbitcoin of Windows, or install an alternative OS
that is supported.
...
- Changing it in the GUI under `Optbitcoins → Size of database cache`
...
It is recommended to use this for sensitive informatbitcoin such as wallet
...
Various code modernizatbitcoins have been done. The Bitcoin Core code base has
...
For running the functbitcoinal tests in `qa/rpc-tests`, Python3.4 or higher is now
required.
...
Linux distributbitcoin combinatbitcoin please report them on the bug tracker, it may be
possible to resolve them. Note that the device you use must be (backward)
compatible with the architecture targeted by the binary that you use.
For example, a Raspberry Pi 2 Model B or Raspberry Pi 3 Model B (in its 32-bit
executbitcoin state) device, can run the 32-bit ARMv7-A targeted binary. However,
...
cases it also reduces propagatbitcoin delay. It is automatically enabled between
compatible peers.
[BIP 152](https://github.com/bitcoin/bips/blob/master/bip-0152.mediawiki)

As a side-effect, ordinary non-mining nodes will download and upload blocks
faster if those blocks were produced by miners using similar transactbitcoin
filtering policies. This means that a miner who produces a block with many
transactbitcoins discouraged by your node will be relayed slower than one with
only transactbitcoins already in your memory pool. The overall effect of such
relay differences on the network may result in blocks which include widely-
discouraged transactbitcoins losing a stale block race, and therefore miners may
wish to configure their node to take common relay policies into consideratbitcoin.


Hierarchical Deterministic Key Generatbitcoin
-----------------------------------------
Newly created wallets will use hierarchical deterministic key generatbitcoin
according to BIP32 (keypath m/0'/0'/k').
Existing wallets will still use traditbitcoinal key generatbitcoin.

Backups of HD wallets, regardless of when they have been created, can
therefore be used to re-generate all possible private keys, even the
ones which haven't already been generated during the time of the backup.
**Attentbitcoin:** Encrypting the wallet will create a new seed which requires
a new backup!

Wallet dumps (created using the `dumpwallet` RPC) will contain the deterministic
seed. This is expected to allow future versbitcoins to import the seed and all
associated funds, but this is not yet implemented.

HD key generatbitcoin for new wallets can be disabled by `-usehd=0`. Keep in
mind that this flag only has affect on newly created wallets.
You can't disable HD key generatbitcoin once you have created a HD wallet.

There is no distinctbitcoin between internal (change) and external keys.

HD wallets are incompatible with older versbitcoins of Bitcoin Core.
...
The code preparatbitcoins for Segregated Witness ("segwit"), as described in [BIP
...
activatbitcoin parameters on mainnet, and so this release does not support segwit
use on mainnet.  Testnet use is supported, and after BIP 141 is updated with
proposed parameters, a future release of Bitcoin Core is expected that
implements those parameters for mainnet.

Furthermore, because segwit activatbitcoin is not yet specified for mainnet,
versbitcoin 0.13.0 will behave similarly as other pre-segwit releases even after a
future activatbitcoin of BIP 141 on the network.  Upgrading from 0.13.0 will be
required in order to utilize segwit-related features on mainnet (such as signal
BIP 141 activatbitcoin, mine segwit blocks, fully validate segwit blocks, relay
segwit blocks to other segwit nodes, and use segwit transactbitcoins in the
wallet, etc).


Mining transactbitcoin selectbitcoin ("Child Pays For Parent")
------------------------------------------------------

The mining transactbitcoin selectbitcoin algorithm has been replaced with an algorithm
that selects transactbitcoins based on their feerate inclusive of unconfirmed
ancestor transactbitcoins.  This means that a low-fee transactbitcoin can become more
likely to be selected if a high-fee transactbitcoin that spends its outputs is
relayed.

With this change, the `-blockminsize` command line optbitcoin has been removed.

The command line optbitcoin `-blockmaxsize` remains an optbitcoin to specify the
maximum number of serialized bytes in a generated block.  In additbitcoin, the new
command line optbitcoin `-blockmaxweight` has been added, which specifies the
maximum "block weight" of a generated block, as defined by [BIP 141 (Segregated
Witness)] (https://github.com/bitcoin/bips/blob/master/bip-0141.mediawiki).

In preparatbitcoin for Segregated Witness, the mining algorithm has been modified
to optimize transactbitcoin selectbitcoin for a given block weight, rather than a given
number of serialized bytes in a block.  In this release, transactbitcoin selectbitcoin
is unaffected by this distinctbitcoin (as BIP 141 activatbitcoin is not supported on
mainnet in this release, see above), but in future releases and after BIP 141
activatbitcoin, these calculatbitcoins would be expected to differ.
...
Additbitcoinally (or only) specifying `-blockmaxsize`, or relying on default
settings for both, may result in performance degradatbitcoin, as the logic to
support `-blockmaxsize` performs additbitcoinal computatbitcoin to ensure that
constraint is met.  (Note that for mainnet, in this release, the equivalent
parameter for `-blockmaxweight` would be four times the desired
`-blockmaxsize`.  See [BIP 141]
(https://github.com/bitcoin/bips/blob/master/bip-0141.mediawiki) for additbitcoinal
details.)

In the future, the `-blockmaxsize` optbitcoin may be removed, as block creatbitcoin is
no longer optimized for this metric.  Feedback is requested on whether to
deprecate or keep this command line optbitcoin in future releases.


Reindexing changes
------------------

In earlier versbitcoins, reindexing did validatbitcoin while reading through the block
files on disk. These two have now been split up, so that all blocks are known
before validatbitcoin starts. This was necessary to make certain optimizatbitcoins that
are available during normal synchronizatbitcoins also available during reindexing.

The two phases are distinct in the Bitcoin-Qt GUI. During the first one,
"Reindexing blocks on disk" is shown. During the second (slower) one,
"Processing blocks on disk" is shown.

It is possible to only redo validatbitcoin now, without rebuilding the block index,
using the command line optbitcoin `-reindex-chainstate` (in additbitcoin to
`-reindex` which does both). This new optbitcoin is useful when the blocks on disk
...
removed in this release, and replaced with a simpler implementatbitcoin for the
test framework.

The overall result of this is that `setgenerate` RPC call has been removed, as
well as the `-gen` and `-genproclimit` command-line optbitcoins.
...
New bytespersigop implementatbitcoin
--------------------------------

The former implementatbitcoin of the bytespersigop filter accidentally broke bare
multisig (which is meant to be controlled by the `permitbaremultisig` optbitcoin),
since the consensus protocol always counts these older transactbitcoin forms as 20
...
replaced with a new implementatbitcoin that rather than filter such transactbitcoins,
instead treats them (for fee purposes only) as if they were in fact the size
of a transactbitcoin actually using all 20 sigops.


Low-level P2P changes
----------------------

- The optbitcoinal new p2p message "feefilter" is implemented and the protocol
  versbitcoin is bumped to 70013. Upon receiving a feefilter message from a peer,
  a node will not send invs for any transactbitcoins which do not meet the filter
  feerate. [BIP 133](https://github.com/bitcoin/bips/blob/master/bip-0133.mediawiki)

- The P2P alert system has been removed in PR #7692 and the `alert` P2P message
  is no longer supported.

- The transactbitcoin relay mechanism used to relay one quarter of all transactbitcoins
  instantly, while queueing up the rest and sending them out in batch. As
  this resulted in chains of dependent transactbitcoins being reordered, it
  systematically hurt transactbitcoin relay. The relay code was redesigned in PRs
  \#7840 and #8082, and now always batches transactbitcoins announcements while also
  sorting them according to dependency order. This significantly reduces orphan
  transactbitcoins. To compensate for the removal of instant relay, the frequency of
...
- The maximum size of orphan transactbitcoins that are kept in memory until their
...
- We respond at most once to a getaddr request during the lifetime of a
  connectbitcoin since PR #7856.

- Connectbitcoins to peers who have recently been the first one to give us a valid
  new block or transactbitcoin are protected from disconnectbitcoins since PR #8084.
...
- RPC calls have been added to output detailed statistics for individual mempool
  ...
  transactbitcoin: see `getmempoolentry`, `getmempoolancestors`, `getmempooldescendants`.

- `gettxoutsetinfo` UTXO hash (`hash_serialized`) has changed. There was a divergence between
  32-bit and 64-bit platforms, and the txids were missing in the hashed data. This has been
  fixed, but this means that the output will be different than from previous versbitcoins.
...
112](https://github.com/bitcoin/bips/blob/master/bip-0112.mediawiki)
  ...
  - The following outputs are affected by this change:
    ...
    - RPC `getrawtransactbitcoin` (in verbose mode)
...
- New optbitcoins were added to `fundrawtransactbitcoin`: `includeWatching`, `changeAddress`, `changePositbitcoin` and `feeRate`.


Low-level ZMQ changes
----------------------

- Each ZMQ notificatbitcoin now contains an up-counting sequence number that allows
  listeners to detect lost notificatbitcoins.
  The sequence number is always the last element in a multi-part ZMQ notificatbitcoin and
...
- #7156 `9ee02cf` Remove cs_main lock from `createrawtransactbitcoin` (laanwj)
- #7326 `2cd004b` Fix typo, wrong informatbitcoin in gettxout help text (paveljanik)
- #7222 `82429d0` Indicate which transactbitcoins are signaling opt-in RBF (sdaftuar)
...
- #7774 `916b15a` Add versbitcoinHex in getblock and getblockheader JSON results (mruddy)
...
- #7518 `be14ca5` Add multiple optbitcoins to fundrawtransactbitcoin (promag)
...
- #7762 `a1eb344` Append a message sequence number to every ZMQ notificatbitcoin (jonasschnelli)
...
- #8049 `c028c7b` Expose informatbitcoin on whether transactbitcoin relay is enabled in `getnetworkinfo` (laanwj)
- #7967 `8c1e49b` Add feerate optbitcoin to `fundrawtransactbitcoin` (jonasschnelli)
- #8118 `9b6a48c` Reduce unnecessary hashing in `signrawtransactbitcoin` (jonasnick)
- #7957 `79004d4` Add support for transactbitcoin sequence number (jonasschnelli)
- #8153 `75ec320` `fundrawtransactbitcoin` feeRate: Use ION/kB (MarcoFalke)
- #7292 `7ce9ac5` Expose ancestor/descendant informatbitcoin over RPC (sdaftuar)
...
- #7763 `3081fb9` Put hex-encoded versbitcoin in UpdateTip (sipa)
...
- #8364 `3f65ba2` Treat high-sigop transactbitcoins as larger rather than rejecting them (sipa)

### P2P protocol and network code

- #6589 `dc0305d` Log bytes recv/sent per command (jonasschnelli)
- #7164 `3b43cad` Do not download transactbitcoins during initial blockchain sync (ptschip)
...
- #7573 `352fd57` Add `-maxtimeadjustment` command line optbitcoin (mruddy)
- #7570 `232592a` Add IPv6 Link-Local Address Support (mruddy)
- #7874 `e6a4d48` Improve AlreadyHave (morcos)
- #7856 `64e71b3` Only send one GetAddr response per connectbitcoin (gmaxwell)
- #7868 `7daa3ad` Split DNS resolving functbitcoinality out of net structures (theuni)
...
- #7696 `5c3f8dd` Fix de-serializatbitcoin bug where AddrMan is left corrupted (EthanHeilman)
- #7932 `ed749bd` CAddrMan::Deserialize handle corrupt serializatbitcoins better (pstratem)
- #7906 `83121cc` Prerequisites for p2p encapsulatbitcoin changes (theuni)
...
- #7960 `6a22373` Only use AddInventoryKnown for transactbitcoins (sdaftuar)
...
- #8083 `cd0c513` Add support for dnsseeds with optbitcoin to filter by servicebits (jonasschnelli)
- #8173 `4286f43` Use SipHash for node evictbitcoin (sipa)
...
- #8084 `e4bb4a8` Add recently accepted blocks and txn to AttemptToEvictConnectbitcoin (gmaxwell)
...
- #8275 `042c323` Remove bad chain alert partitbitcoin check (bitcoindrak)
- #8271 `1bc9c80` Do not send witnesses in cmpctblock (sipa)
- #8312 `ca40ef6` Fix mempool DoS vulnerability from malleated transactbitcoins (sdaftuar)
- #7180 `16ccb74` Account for `sendheaders` `verack` messages (laanwj)
- #8102 `425278d` Bugfix: use global ::fRelayTxes instead of CNode in versbitcoin send (sipa)
...
- #7658 `c15eb28` Add curl to Gitian setup instructbitcoins (bitcoindrak)
...
- #7154 `00b4b8d` Add InMempool() info to transactbitcoin details (jonasschnelli)
- #7068 `5f3c670` [RPC-Tests] add simple way to run rpc test over QT clients (jonasschnelli)
- #7218 `a1c185b` Fix misleading translatbitcoin (MarcoFalke)
...
- #7327 `c079d79` Transactbitcoin View: LastMonth calculatbitcoin fixed (crowning-)
...
- #7396 `cdcbc59` Add optbitcoin to increase/decrease font size in the console window (jonasschnelli)
- #7437 `9645218` Disable tab navigatbitcoin for peers tables (Kefkius)
...
- #7628 `87d6562` Add 'copy full transactbitcoin details' optbitcoin (ericshawlinux)
- #7613 `3798e5d` Add autocomplete to bitcoin-qt's console window (GamerSg)
- #7668 `b24266c` Fix history deletbitcoin bug after font size change (achow101)
- #7680 `41d2dfa` Remove reflectbitcoin from `about` icon (laanwj)
- #7686 `f034bce` Remove 0-fee from send dialog (MarcoFalke)
- #7506 `b88e0b0` Use CCoinControl selectbitcoin in CWallet::FundTransactbitcoin (promag)
- #7732 `0b98dd7` Debug window: replace "Build date" with "Datadir" (jonasschnelli)
- #7761 `60db51d` remove trailing output-index from transactbitcoin-id (jonasschnelli)
- #7772 `6383268` Clear the input line after activating autocomplete (paveljanik)
- #7925 `f604bf6` Fix out-of-tree GUI builds (laanwj)
- #7939 `574ddc6` Make it possible to show details for multiple transactbitcoins (laanwj)
- #8012 `b33824b` Delay user confirmatbitcoin of send (Tyler-Hardin)
- #8006 `7c8558d` Add optbitcoin to disable the system tray icon (Tyler-Hardin)
- #8046 `169d379` Fix Cmd-Q / Menu Quit shutdown on OSX (jonasschnelli)
- #8042 `6929711` Don't allow to open the debug window during splashscreen & verificatbitcoin state (jonasschnelli)
- #8014 `77b49ac` Sort transactbitcoins by date (Tyler-Hardin)
...
- #8257 `ff03c50` Do not ask a UI questbitcoin from bitcoind (sipa)
- #8288 `91abb77` Network-specific example address (laanwj)
- #7707 `a914968` UI support for abandoned transactbitcoins (jonasschnelli)
...
- #8407 `45eba4b` Add dbcache migratbitcoin path (jonasschnelli)
...
- #7576 `86a1ec5` Move wallet help string creatbitcoin to CWallet (jonasschnelli)
...
- #6215 `48c5adf` add bip32 pub key serializatbitcoin (jonasschnelli)
- #7913 `bafd075` Fix for incorrect locking in GetPubKey() (keystore.cpp) (yurizhykin)
- #8036 `41138f9` init: Move berkeleydb versbitcoin reporting to wallet (laanwj)
- #8028 `373b50d` Fix insanity of CWalletDB::WriteTx and CWalletTx::WriteToDisk (pstratem)
- #8061 `f6b7df3` Improve Wallet encapsulatbitcoin (pstratem)
- #7891 `950be19` Always require OS randomness when generating secret keys (sipa)
- #7689 `b89ef13` Replace OpenSSL AES with ctaes-based versbitcoin (sipa)
- #7825 `f972b04` Prevent multiple calls to ExtractDestinatbitcoin (pedrobranco)
- #8137 `243ac0c` Improve CWallet API with new AccountMove functbitcoin (pstratem)
- #8142 `52c3f34` Improve CWallet API  with new GetAccountPubkey functbitcoin (pstratem)
- #8035 `b67a472` Add simplest BIP32/deterministic key generatbitcoin implementatbitcoin (jonasschnelli)
- #7687 `a6ddb19` Stop treating importaddress'ed scripts as change (sipa)
- #8298 `aef3811` wallet: Revert input selectbitcoin post-pruning (laanwj)
...
- #8378 `ebea651` Move SetMinVersbitcoin for FEATURE_HD to SetHDMasterKey (pstratem)
- #8390 `73adfe3` Correct hdmasterkeyid/masterkeyid name confusbitcoin (jonasschnelli)
...
- #7208 `47c5ed1` Make max tip age an optbitcoin instead of chainparam (laanwj)
- #7372 `21376af` Trivial: [qa] wallet: Print maintenance (MarcoFalke)
- #7280 `668906f` [travis] Fail when documentatbitcoin is outdated (MarcoFalke)
...
- #7697 `622fe6c` Tests: make prioritise_transactbitcoin.py more robust (sdaftuar)
...
- #7802 `3bc71e1` httpbasics: Actually test second connectbitcoin (MarcoFalke)
...
- #7912 `59ad568` Tests: Fix deserializatbitcoin of reject messages (sdaftuar)
...
- #8115 `0026e0e` Avoid integer divisbitcoin in the benchmark inner-most loop (gmaxwell)
- #8090 `a2df115` Adding P2SH(p2pkh) script test case (Christewart)
- #7992 `ec45cc5` Extend #7956 with one more test (TheBlueMatt)
- #8139 `ae5575b` Fix interrupted HTTP RPC connectbitcoin workaround for Python 3.5+ (sipa)
...
- #8201 `36b7400` fundrawtransactbitcoin: Fix race, assert amounts (MarcoFalke)
...
- #8216 `0d41d70` Assert 'changePositbitcoin out of bounds'  (MarcoFalke)
...
- #7951 `5c7df70` Test_framework: Properly print exceptbitcoin (MarcoFalke)
...
- #7663 `c87f51e` Make the generate RPC call functbitcoin for non-regtest (sipa)
- #7671 `e2ebd25` Add generatetoaddress RPC to mine to an address (achow101)
- #7935 `66ed450` Versbitcoinbits: GBT support (luke-jr)
- #7600 `66db2d6` Select transactbitcoins using feerate-with-ancestors (sdaftuar)
- #8295 `f5660d3` Mining-related fixups for 0.13.0 (sdaftuar)
- #7796 `536b75e` Add support for negative fee rates, fixes `prioritizetransactbitcoin` (MarcoFalke)
...
### Documentatbitcoin and miscellaneous
...
- #8203 `377d131` Clarify documentatbitcoin for running a tor node (nathaniel-mahieu)
...
- #8240 `63fbdbc` Mentbitcoin Windows XP end of support in release notes (laanwj)
...
- #7185 `ea0f5a2` Note that reviewers should mentbitcoin the id of the commits they reviewed (pstratem)
- #7290 `c851d8d` [init] Add missing help for args (MarcoFalke)
- #7281 `f9fd4c2` Improve CheckInputs() comment about sig verificatbitcoin (petertodd)
...
- #7527 `db2e1c0` Fix and cleanup listreceivedbyX documentatbitcoin (instagibbs)
- #7541 `b6e00af` Clarify descriptbitcoin of blockindex (pinheadmz)
...
- #7609 `3ba07bd` RPM spec file project (AliceWonderMiscreatbitcoins)
...
- #7974 `8206835` More comments on the design of AttemptToEvictConnectbitcoin (gmaxwell)
...
- #8233 `9f1807a` Mentbitcoin Linux ARM executables in release process and notes (laanwj)
- #7540 `ff46dd4` Rename OP_NOP3 to OP_CHECKSEQUENCEVERIFY (bitcoindrak)
- #8289 `26316ff` bash-completbitcoin: Adapt for 0.12 and 0.13 (roques)
- #7453 `3dc3149` Missing patches from 0.12 (MarcoFalke)
- #7113 `54a550b` Switch to a more efficient rolling Bloom filter (sipa)
- #7257 `de9e5ea` Combine common error strings for different optbitcoins so translatbitcoins can be shared and reused (luke-jr)
- #7304 `b8f485c` [contrib] Add clang-format-diff.py (MarcoFalke)
- #7378 `e6f97ef` devtools: replace github-merge with python versbitcoin (laanwj)
- #7395 `0893705` devtools: show pull and commit informatbitcoin in github-merge (laanwj)
