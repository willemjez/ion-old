Bitcoin Core versbitcoin 0.14.0 is now available from:

  <https://bitcoin.org/bin/bitcoin-core-0.14.0/>

This is a new major versbitcoin release, including new features, various bugfixes
and performance improvements, as well as updated translatbitcoins.
...
Validatbitcoin speed and network propagatbitcoin performance have been greatly
...
- Assumed-valid blocks have been introduced which allows script validatbitcoin to
...
- P2P networking has been refactored with a focus on concurrency and
  throughput. Network operatbitcoins are no longer bottlenecked by validatbitcoin. As a
...
additbitcoinal informatbitcoin that `getinfo` did not provide. The following table
shows where each field has been moved to:

|`getinfo` field   | Moved to                                  |
|------------------|-------------------------------------------|
`"versbitcoin"`	   | `getnetworkinfo()["versbitcoin"]`
`"protocolversbitcoin"`| `getnetworkinfo()["protocolversbitcoin"]`
`"walletversbitcoin"`  | `getwalletinfo()["walletversbitcoin"]`
`"balance"`	   | `getwalletinfo()["balance"]`
`"blocks"`	   | `getblockchaininfo()["blocks"]`
`"timeoffset"`	   | `getnetworkinfo()["timeoffset"]`
`"connectbitcoins"`	   | `getnetworkinfo()["connectbitcoins"]`
...
Previously the ZeroMQ notificatbitcoin system was unavailable on Windows
...
Additbitcoinally the `setnetworkactive` RPC command has been added which does
...
When Bitcoin Core is out-of-sync on startup, a semi-transparent informatbitcoin
...
can now use named arguments. This follows the [JSON-RPC specificatbitcoin](http://www.jsonrpc.org/specificatbitcoin)
for passing parameters by-name with an object.

`bitcoin-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` optbitcoin is given.
...
The RPC server remains fully backwards compatible with positbitcoinal arguments.

Opt into RBF When Sending
-------------------------

A new startup optbitcoin, `-walletrbf`, has been added to allow users to have all
transactbitcoins sent opt into RBF support. The default value for this optbitcoin is
currently `false`, so transactbitcoins will not opt into RBF by default. The new
`bumpfee` RPC can be used to replace transactbitcoins that opt into RBF.
...
entered commands. Commands which have sensitive informatbitcoin such as passphrases and
...
restarts the mempool can be filled with transactbitcoins without waiting for new transactbitcoins
to be created. This will also preserve any changes made to a transactbitcoin through
commands such as `prioritisetransactbitcoin` so that those changes will not be lost.
...
-----------
 ...
 - After resetting the optbitcoins by clicking the `Reset Optbitcoins` button 
   in the optbitcoins dialog or with the `-resetguioptbitcoins` startup optbitcoin, 
   the user will be prompted to choose the data directory again. This 
   is to ensure that custom data directories will be kept after the 
   optbitcoin reset which clears the custom data directory set via the choose 
   datadir dialog.
...
----------------------
 ...
 - `importprunedfunds` only accepts two required arguments. Some versbitcoins accept
   an optbitcoinal third arg, which was always ignored. Make sure to never pass more
   than two arguments.
 ...
 - A new RPC command `importmulti` has been added which receives an array of 
   JSON objects representing the intentbitcoin of importing a public key, a 
   private key, an address and script/p2sh

 - Use of `getrawtransactbitcoin` for retrieving confirmed transactbitcoins with unspent
   outputs has been deprecated. For now this will still work, but in the future
   it may change to only be able to retrieve informatbitcoin about transactbitcoins in
   the mempool or if `txindex` is enabled.

 - A new RPC command `getmemoryinfo` has been added which will return informatbitcoin
   about the memory usage of Bitcoin Core. This was added in conjunctbitcoin with
   optimizatbitcoins to memory management. See [Pull #8753](https://github.com/bitcoin/bitcoin/pull/8753)
   for more informatbitcoin.

 - A new RPC command `bumpfee` has been added which allows replacing an
   unconfirmed wallet transactbitcoin that signaled RBF (see the `-walletrbf`
   startup optbitcoin above) with a new transactbitcoin that pays a higher fee, and
...
Since the changes in 0.12 to automatically limit the size of the mempool and improve the performance of block creatbitcoin in mining code it has not been important for relay nodes or miners to set `-minrelaytxfee`. With this release the following concepts that were tied to this optbitcoin have been separated out:
- incremental relay fee used for calculating BIP 125 replacement and mempool limiting. (1000 satoshis/kB)
- calculatbitcoin of threshold for a dust output. (effectively 3 * 1000 satoshis/kB)
- minimum fee rate of a package of transactbitcoins to be included in a block created by the mining code. If miners wish to set this minimum they can use the new `-blockmintxfee` optbitcoin.  (defaults to 1000 satoshis/kB)

The `-minrelaytxfee` optbitcoin continues to exist but is recommended to be left unset.

Fee Estimatbitcoin Changes
----------------------

- Since 0.13.2 fee estimatbitcoin for a confirmatbitcoin target of 1 block has been
...
- The default target for fee estimatbitcoin is changed to 6 blocks in both the GUI
  (previously 25) and for RPC calls (previously 2).

Removal of Priority Estimatbitcoin
------------------------------

- Estimatbitcoin of "priority" needed for a transactbitcoin to be included within a target
  number of blocks has been removed.  The RPC calls are deprecated and will either
  return -1 or 1e24 appropriately. The format for `fee_estimates.dat` has also
  changed to no longer save these priority estimates. It will automatically be
  converted to the new format which is not readable by prior versbitcoins of the
  software.

- Support for "priority" (coin age) transactbitcoin sorting for mining is
  considered deprecated in Core and will be removed in the next major versbitcoin.
  This is not to be confused with the `prioritisetransactbitcoin` RPC which will remain
  supported by Core for adding fee deltas to transactbitcoins.

P2P connectbitcoin management
--------------------------

- Peers manually added through the `-addnode` optbitcoin or `addnode` RPC now have their own
  limit of eight connectbitcoins which does not compete with other inbound or outbound
  connectbitcoin usage and is not subject to the limitatbitcoin imposed by the `-maxconnectbitcoins`
  optbitcoin.

- New connectbitcoins to manually added peers are performed more quickly.

Introductbitcoin of assumed-valid blocks
-------------------------------------

- A significant portbitcoin of the initial block download time is spent verifying
  scripts/signatures.  Although the verificatbitcoin must pass to ensure the security
  of the system, no other result from this verificatbitcoin is needed: If the node
  knew the history of a given block were valid it could skip checking scripts
  for its ancestors.

- A new configuratbitcoin optbitcoin 'assumevalid' is provided to express this knowledge
...
Fundrawtransactbitcoin change address reuse
----------------------------------------

- Before 0.14, `fundrawtransactbitcoin` was by default wallet stateless. In
  almost all cases `fundrawtransactbitcoin` does add a change-output to the
  outputs of the funded transactbitcoin. Before 0.14, the used keypool key was
  ...
  directly after `fundrawtransactbitcoin` did generate the same address as
  the change-output address.

- Since 0.14, fundrawtransactbitcoin does reserve the change-output-key from
  the keypool by default (optbitcoinal by setting  `reserveChangeKey`, default =
  `true`)

- Users should also consider using `getrawchangeaddress()` in conjunctbitcoin
  with `fundrawtransactbitcoin`'s `changeAddress` optbitcoin.

Unused mempool memory used by coincache
----------------------------------------

- Before 0.14, memory reserved for mempool (using the `-maxmempool` optbitcoin)
  went unused during initial block download, or IBD. In 0.14, the UTXO DB cache
  (controlled with the `-dbcache` optbitcoin) borrows memory from the mempool
  when there is extra memory available. This may result in an increase in
  memory usage during IBD for those previously relying on only the `-dbcache`
  optbitcoin to limit memory during that time.
...
in locating the code changes and accompanying discussbitcoin, both the pull request
and git merge commit are mentbitcoined.
...
- #8272 `91990ee` Make the dummy argument to getaddednodeinfo optbitcoinal (sipa)
- #8722 `bb843ad` bitcoin-cli: More detailed error reporting (laanwj)
- #6996 `7f71a3c` Add preciousblock RPC (sipa)
- #8788 `97c7f73` Give RPC commands more informatbitcoin about the RPC request (jonasschnelli)
- #7948 `5d2c8e5` Augment getblockchaininfo bip9\_softforks data (mruddy)
- #8980 `0e22855` importmulti: Avoid using boost::variant::operator!=, which is only in newer boost versbitcoins (luke-jr)
- #9025 `4d8558a` Getrawtransactbitcoin should take a bool for verbose (jnewbery)
- #8811 `5754e03` Add support for JSON-RPC named arguments (laanwj)
- #9520 `2456a83` Deprecate non-txindex getrawtransactbitcoin and better warning (sipa)
- #9518 `a65ced1` Return height of last block pruned by pruneblockchain RPC (ryanofsky)
- #9222 `7cb024e` Add 'subtractFeeFromAmount' optbitcoin to 'fundrawtransactbitcoin' (dooglus)
...
- #9640 `7bfb770` Bumpfee: bugfixes for error handling and feerate calculatbitcoin (sdaftuar)
- #9673 `8d6447e` Set correct metadata on bumpfee wallet transactbitcoins (ryanofsky)
- #9650 `40f7e27` Better handle invalid parameters to signrawtransactbitcoin (TheBlueMatt)
...
- #8365 `618c9dd` Treat high-sigop transactbitcoins as larger rather than rejecting them (sipa)
- #8814 `14b7b3f` wallet, policy: ParameterInteractbitcoin: Don't allow 0 fee (MarcoFalke)
- #8515 `9bdf526` A few mempool removal optimizatbitcoins (sipa)
- #8448 `101c642` Store mempool and prioritizatbitcoin data to disk (sipa)
- #7730 `3c03dc2` Remove priority estimatbitcoin (morcos)
...
- #7562 `1eef038` Bump transactbitcoin versbitcoin default to 2 (bitcoindrak)
...
- #9138 `f646275` Improve fee estimatbitcoin (morcos)
...
- #9519 `9b4d267` Exclude RBF replacement txs from fee estimatbitcoin (morcos)
- #8606 `e2a1a1e` Fix some locks (sipa)
- #8681 `6898213` Performance Regressbitcoin Fix: Pre-Allocate txChanged vector (JeremyRubin)
- #8223 `744d265` c++11: Use std::unique\_ptr for block creatbitcoin (domob1812)
- #9125 `7490ae8` Make CBlock a vector of shared\_ptr of CTransactbitcoins (sipa)
- #8930 `93566e0` Move orphan processing to ActivateBestChain (TheBlueMatt)
- #8580 `46904ee` Make CTransactbitcoin actually immutable (sipa)
...
- #9283 `869781c` A few more CTransactbitcoinRef optimizatbitcoins (sipa)
- #9499 `9c9af5a` Use recent-rejects, orphans, and recently-replaced txn for compact-block-reconstructbitcoin (TheBlueMatt)
...
- #8282 `026c6ed` Feeler connectbitcoins to increase online addrs in the tried table (EthanHeilman)
...
- #8085 `6423116` Begin encapsulatbitcoin (theuni)
...
- #9002 `e1d1f57` Make connect=0 disable automatic outbound connectbitcoins (gmaxwell)
- #9050 `fcf61b8` Make a few values immutable, and use deterministic randomness for the localnonce (theuni)
- #8969 `3665483` Decouple peer-processing-logic from block-connectbitcoin-logic (#2) (TheBlueMatt)
...
- #9128 `76fec09` Decouple CConnman and message serializatbitcoin (theuni)
- #9226 `3bf06e9` Remove fNetworkNode and pnodeLocalHost (gmaxwell)
- #9352 `a7f7651` Attempt reconstructbitcoin from all compact block announcements (sdaftuar)
- #9319 `a55716a` Break addnode out from the outbound connectbitcoin limits (gmaxwell)
...
- #9375 `3908fc4` Relay compact block messages prior to full block connectbitcoin (TheBlueMatt)
- #9400 `8a445c5` Set peers as HB peers upon full block validatbitcoin (instagibbs)
...
- #9609 `4966917` Fix remaining net assertbitcoins (theuni)
...
- #9075 `9346f84` Decouple peer-processing-logic from block-connectbitcoin-logic (#3) (TheBlueMatt)
- #8688 `047ded0` Move static global randomizer seeds into CConnman (sipa)
- #9289 `d9ae1ce` net: drop boost::thread\_group (theuni)

### Validatbitcoin
- #9014 `d04aeba` Fix block-connectbitcoin performance regressbitcoin (TheBlueMatt)
...
- #8895 `b83264d` Better SigCache Implementatbitcoin (JeremyRubin)
- #9490 `e126d0c` Replace FindLatestBefore used by importmulti with FindEarliestAtLeast (gmaxwell)
- #9484 `812714f` Introduce assumevalid setting to skip validatbitcoin presumed valid scripts (gmaxwell)
- #9511 `7884956` Don't overwrite validatbitcoin state with corruptbitcoin check (morcos)
...
- #8617 `de07fdc` Include instructbitcoins to extract Mac OS X SDK on Linux using 7zip and SleuthKit (luke-jr)
...
- #8781 `ca69ef4` contrib: delete `qt_translatbitcoins.py` (MarcoFalke)
...
- #8820 `dc64141` depends: Fix Qt compilatbitcoin with Xcode 8 (fanquake)
...
- #9156 `a8b2a82` Add compile and link optbitcoins echo to configure (jonasschnelli)
...
- #9705 `6a55515` Add optbitcoins to override BDB cflags/libs (laanwj)
- #8249 `4e1567a` Enable (and check for) 64-bit ASLR on Windows (laanwj)
- #9758 `476cc47` Selectively suppress deprecatbitcoin warnings (jonasschnelli)
...
### GUI
- #8192 `c503863` Remove URLs from About dialog translatbitcoins (fanquake)
...
- #8672 `6052d50` Show transactbitcoin size in transactbitcoin details window (Cocosoft)
...
- #8989 `d2143dc` Overhaul smart-fee slider, adjust default confirmatbitcoin target (jonasschnelli)
...
- #8874 `e984730` Multiple Selectbitcoin for peer and ban tables (achow101)
- #9145 `924745d` Make network disabled icon 50% opaque (MarcoFalke)
- #9130 `ac489b2` Mentbitcoin the new network toggle functbitcoinality in the tooltip (paveljanik)
...
- #9457 `123ea73` Select more files for translatbitcoin (MarcoFalke)
...
- #9637 `d9e4d1d` Fix transactbitcoin details output-index to reflect vout index (jonasschnelli)
- #9718 `36f9d3a` Qt/Intro: Various fixes (luke-jr)
- #9735 `ec66d06` devtools: Handle Qt formatting characters edge-case in update-translatbitcoins.py (laanwj)
- #9755 `a441db0` Bugfix: Qt/Optbitcoins: Restore persistent "restart required" notice (luke-jr)
...
- #8445 `f916700` Move CWallet::setKeyPool to private sectbitcoin of CWallet (pstratem)
- #8564 `0168019` Remove unused code/conditbitcoins in ReadAtCursor (jonasschnelli)
- #8601 `37ac678` Add optbitcoin to opt into full-RBF when sending funds (rebase, original by petertodd) (laanwj)
- #8494 `a5b20ed` init, wallet: ParameterInteractbitcoin() iff wallet enabled (MarcoFalke)
...
- #8486 `ab0b411` Add high transactbitcoin fee warnings (MarcoFalke)
- #8851 `940748b` Move key derivatbitcoin logic from GenerateNewKey to DeriveNewChildKey (pstratem)
...
- #8828 `a4fd8df` Move CWalletDB::ReorderTransactbitcoins to CWallet (pstratem)
- #8977 `6a1343f` Refactor wallet/init interactbitcoin (Reaccept wtx, flush thread) (jonasschnelli)
- #9036 `ed0cc50` Change default confirm target from 2 to 6 (laanwj)
- #9071 `d1871da` Declare wallet.h functbitcoins inline (sipa)
...
- #9311 `a336d13` Flush wallet after abandontransactbitcoin (morcos)
- #8717 `38e4887` Additbitcoin of ImmatureCreditCached to MarkDirty() (spencerlievens)
...
- #9465 `a7d55c9` Do not perform ECDSA signing in the fee calculatbitcoin inner loop (gmaxwell)
- #9404 `12e3112` Smarter coordinatbitcoin of change and fee in CreateTransactbitcoin (morcos)
- #9377 `fb75cd0` fundrawtransactbitcoin: Keep change-output keys by default, make it optbitcoinal (jonasschnelli)
...
- #9764 `f8af89a` Prevent "overrides a member functbitcoin but is not marked 'override'" warnings (laanwj)
- #9771 `e43a585` Add missing cs\_wallet lock that triggers new lock held assertbitcoin (ryanofsky)
- #9316 `3097ea4` Disable free transactbitcoins when relay is disabled (MarcoFalke)
...
- #8810 `14e8f99` tests: Add exceptbitcoin error message for JSONRPCExceptbitcoin (laanwj)
- #8830 `ef0801b` test: Add optbitcoin to run bitcoin-util-test.py manually (jnewbery)
...
- #8894 `faec09b` Testing: Include fRelay in mininode versbitcoin messages (jnewbery)
- #9097 `e536499` Rework `sync_*` and preciousblock.py (MarcoFalke)
- #9049 `71bc39e` Remove duplicatable duplicate-input check from CheckTransactbitcoin (TheBlueMatt)
...
- #9223 `5412c08` unificatbitcoin of Bloom filter representatbitcoin (s-matthew-english)
- #9257 `d7ba4a2` Dump debug logs on travis failures (sdaftuar)
- #9221 `9e4bb31` Get rid of duplicate code (MarcoFalke)
- #9274 `919db03` Use cached utxo set to fix performance regressbitcoin (MarcoFalke)
- #9276 `ea33f19` Some minor testing cleanups (morcos)
- #9291 `8601784` Remove mapOrphanTransactbitcoinsByPrev from DoS\_tests (sipa)
...
- #9638 `a7ea2f8` Actually test assertbitcoins in pruning.py (MarcoFalke)
- #9647 `e99f0d7` Skip RAII event tests if libevent is built without `event_set_mem_functbitcoins` (luke-jr)
- #9691 `fc67cd2` Init ECC context for `test_bitcoin_fuzzy` (gmaxwell)
- #9712 `d304fef` bench: Fix initializatbitcoin order in registratbitcoin (laanwj)
- #9707 `b860915` Fix RPC failure testing (jnewbery)
- #9269 `43e8150` Align struct COrphan definitbitcoin (sipa)
...
### Documentatbitcoin
- #8332 `806b9e7` Clarify witness branches in transactbitcoin.h serializatbitcoin (dcousens)
- #8935 `0306978` Documentatbitcoin: Building on Windows with WSL (pooleja)
...
- #9378 `870cd2b` Add documentatbitcoin for CWalletTx::fFromMe member (ryanofsky)
- #9297 `0b73807` Various RPC help outputs updated (Mirobit)
- #9613 `07421cf` Clarify getbalance help string to explain interactbitcoin with bumpfee (ryanofsky)
- #9663 `e30d928` Clarify listunspent amount descriptbitcoin (instagibbs)
- #9396 `d65a13b` Updated listsinceblock rpc documentatbitcoin (accraze)
- #8747 `ce43630` rpc: Fix transactbitcoin size comments and RPC help text (jnewbery)
- #8058 `bbd9740` Doc: Add issue template (AmirAbrams)
- #8567 `85d4e21` Add default port numbers to REST doc (djpnewton)
- #8624 `89de153` build: Mentbitcoin curl (MarcoFalke)
- #8786 `9da7366` Mandatory copyright agreement (achow101)
- #8823 `7b05af6` Add privacy recommendatbitcoin when running hidden service (laanwj)
...
- #8892 `d270c30` Add build instructbitcoins for FreeBSD (laanwj)
- #8890 `c71a654` Update Doxygen configuratbitcoin file (fanquake)
- #9207 `fa1f944` Move comments above bash command in build-unix (AmirAbrams)
- #9219 `c4522e7` Improve windows build instructbitcoins using Linux subsystem (laanwj)
- #8954 `932d02a` contrib: Add README for pgp keys (MarcoFalke)
- #9093 `2fae5b9` release-process: Mentbitcoin GitHub release and archived release notes (MarcoFalke)
...
- #8771 `bc9e3ab` contributing: Mentbitcoin not to open several pulls (luke-jr)
- #8852 `7b784cc` Mentbitcoin Gitian building script in doc (Laudaa) (laanwj)
- #8915 `03dd707` Add copyright/patent issues to possible NACK reasons (petertodd)
- #8965 `23e03f8` Mentbitcoin that PPA doesn't support Debian (anduck)
- #9115 `bfc7aad` Mentbitcoin reporting security issues responsibly (paveljanik)
- #9840 `08e0690` Update sendfrom RPC help to correct coin selectbitcoin misconceptbitcoin (ryanofsky)
- #9865 `289204f` Change bitcoin address in RPC help message (marijnfs)

### Miscellaneous
- #8274 `7a2d402` util: Update tinyformat (laanwj)
- #8291 `5cac8b1` util: CopyrightHolders: Check for untranslated substitutbitcoin (MarcoFalke)
...
- #9004 `67728a3` Clarify `listenonbitcoin` (unsystemizer)
- #8674 `bae81b8` tools for analyzing, updating and adding copyright headers in source files (isle2983)
- #8976 `8c6218a` libconsensus: Add input validatbitcoin of flags (laanwj)
- #9112 `46027e8` Avoid ugly exceptbitcoin in log on unknown inv type (laanwj)
- #8837 `2108911` Allow bitcoin-tx to parse partial transactbitcoins (jnewbery)
- #9204 `74ced54` Clarify CreateTransactbitcoin error messages (instagibbs)
...
- #9506 `593a00c` RFC: Improve style for if indentatbitcoin (sipa)
- #8883 `d5d4ad8` Add all standard TXO types to bitcoin-tx (jnewbery)
- #9531 `23281a4` Release notes for estimatbitcoin changes  (morcos)
...
- #9039 `e81df49` Various serializatbitcoin simplifcatbitcoins and optimizatbitcoins (sipa)
...
- #9236 `7f72568` Fix races for strMiscWarning and `fLargeWork*Found`, make QT runawayExceptbitcoin use GetWarnings (gmaxwell)
...
- #9472 `fac0f30` Disentangle progress estimatbitcoin from checkpoints and update it (sipa)
- #9512 `6012967` Fix various things -fsanitize complains about (sipa)
- #9373,#9580 Various linearizatbitcoin script issues (droark)
...
- #9856 `69832aa` Terminate immediately when allocatbitcoin fails (theuni)
